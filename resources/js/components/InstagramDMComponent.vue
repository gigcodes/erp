  ...
  <div>
    <div class="actions">
      <div class="row">
        ...
        <div class="col-md-4 text-right">
          <a href="/cold-leads-broadcasts" class="btn btn-primary btn-sm"
            >Broadcasts</a
          >
        </div>
      </div>
    ...
    <table class="table table-striped table-sm table-bordered mt-4">
      ...
      <tbody>
        <tr v-if="isLoading">
          <td colspan="6">
            <h1 class="text-center">Loading cold leads...</h1>
          </td>
        </tr>
        <tr v-for="(lead, index) in leads">
          ...
          <td>
            ...
            <strong>IG Profile :</strong>
            <a :href="'https://instagram.com/' + lead.username">Visit Profile</a
            ><br />
          ...
          <td>
            <div
              :key="'chat_' + lead.id + '_' + Math.floor(Math.random() * 100)"
              class="card"
            ...
            >
              <div class="card-header">
                ...
                <div class="pull-right">
                  <div class="form-group form-group-sm">
                    <strong>{{
                      lead.account_id ? lead.account.last_name : "N/A"
                    }}</strong>
                  </div>
                </div>
              </div>
              <div class="card-body">
                <div
                  class="chats text-center mb-4"
                  v-if="!allDirectMessages.hasOwnProperty(lead.id)"
                >
                ...
                >
                  <div class="row">
                    <div class="col-md-12">
                      <div
                        class="text-center"
                        v-if="allDirectMessages[lead.id].length == 0"
                      >
                      ...
                      <template v-if="allDirectMessages[lead.id].length > 0">
                        ...
                        >
                          <div
                            v-for="msg in leadActiveMessages[lead.id][
                              'messages'
                            ]"
                          >
                            <div
                              :class="{
                                balon2:
                                  leadActiveMessages[lead.id]
                                    .lead_instagram_id == msg.sender_id,
                                balon1:
                                  leadActiveMessages[lead.id]
                                    .lead_instagram_id != msg.sender_id,
                              }"
                              class="is-received p-2 m-0 position-relative"
                            >
                              <a
                                :class="{
                                  'float-right':
                  ...
                  <div class="row">
                    <div class="col-md-12">
                      <div class="form-group">
                        <input
                          :disabled="currentDisabledLead == lead.id"
                          v-model.lazy="message[lead.id]"
                          :key="index + 1"
                    ...
                    <div class="col-md-12 text-right">
                      <img
                        v-if="currentDisabledLead == lead.id"
                        src="/images/loading2.gif"
                        style="width: 40px"
                      />
                      <label
                        @click="setCurrentLead(lead.id)"
                        for="image"
                        class="btn btn-sm btn-primary"
                      >
          ...
          <td>
            <button
              @click="addColdLeadToCustomer(lead.id)"
              class="btn btn-sm btn-success mt-2"
            >
              <i class="fa fa-plus"></i>
            </button>
            <button
              @click="deleteColdLead(lead.id)"
...
export default {
  ...
  methods: {
    ...
    sendImage() {
      ...
      data.append(
        "account_id",
        this.accounts[this.selectedAccounts[leadId]].id
      );
