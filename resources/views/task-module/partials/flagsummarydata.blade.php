@foreach ($tasks as $key => $task)
    <?php $special_task = \App\Task::find($task->id); ?>
    <tr style="color:grey;">
        ...
        <td>
            <div class="d-flex">
                ...
                <button style="float:right;padding-right:0px; background: none;" type="button"
                    class="btn btn-xs show-user-history" title="Show History" data-id="{{ $task->id }}"
        ...
        <td style="vertical-align: baseline;">
            <div class="d-flex">
                <input type="text" class="form-control send-message-textbox" data-id="{{ $task->id }}"
                    id="send_message_{{ $task->id }}" name="send_message_{{ $task->id }}"
                ...
                <button style="display: inline-block;padding:0px;" class="btn btn-sm btn-image send-message-open"
                    type="submit" id="submit_message" data-id="{{ $task->id }}" style="width:10%"><img
                        src="/images/filled-sent.png" /></button>
            </div>
            <div class="d-flex">
                <button type="button" class="btn btn-xs btn-image load-communication-modal" data-object='task'
                    data-id="{{ $task->id }}" style="mmargin-top: -0%;margin-left: -2%; width:10%"
                    title="Load messages"><img src="/images/chat.png" alt=""></button>
                <span
                    class="Website-task pr-0 {{ ($task->message && $task->message_status == 0) ||$task->message_is_reminder == 1 ||($task->sent_to_user_id == Auth::id() && $task->message_status == 0)? '': '' }} justify-content-between expand-row-msg"
                ...
                <span class="td-full-container-{{ $task->id }} hidden">
                    ...
                    <div class="td-full-container">
                        ...
                        <button class="btn btn-secondary btn-xs" onclick="sendUploadImage({{ $task->id }})">Send
                            Images</button>
                        <input id="file-input{{ $task->id }}" type="file" name="files" style="display: none;"
                            multiple />
                    </div>
                </span>
            </div>
            ...
            <p>
                ...
                @if ($task->is_flagged == 1)
                    <button type="button" class="btn pr-0 btn-image flag-task pd-5" data-type="task"
                        data-id="{{ $task->id }}"><img src="{{ asset('images/flagged.png') }}" /
