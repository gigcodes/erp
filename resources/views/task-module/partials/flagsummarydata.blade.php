...
<tr style="color:grey;">
    ...
    <td style="vertical-align: baseline;">
        ...
        <a data-toggle="modal" data-target="#task_subject{{ $issue->id }}" class="btn pd-5 task-set-reminder"
    ...
    <td>
        <div class="d-flex">
            <select class="form-control assign-task-user select2" data-id="{{ $issue->id }}" name="assign_to"
                id="user_{{ $issue->id }}">
            ...
            <button style="float:right;padding-right:0px; background: none;" type="button"
                class="btn btn-xs show-user-history" title="Show History" data-id="{{ $issue->id }}"
    ...
    <td style="vertical-align: baseline;">
        ...
        <button type="button" style="float:right;" class="btn btn-xs show-time-history-task" title="Show History"
            data-id="{{ $issue->id }}" data-user_id="{{ $issue->assign_to }}" style="background: none;"><i
                class="fa fa-info-circle"></i></button>
    </td>
    <td style="vertical-align: baseline;">
        <div class="d-flex">
            ...
            <button type="button" class="btn btn-xs show-date-history" title="Show tracked time History"
                data-id="{{ $issue->id }}" data-type="task" style="float:right;margin-left: auto;"><i
                    class="fa fa-info-circle"></i></button>
        </div>
    </td>
    ...
    <td style="vertical-align: baseline;">
        <div class="d-flex">
            <input type="text" class="form-control send-message-textbox" data-id="{{ $issue->id }}"
                ...
                style="margin-bottom:5px;width:calc(100% - 24px);display:inline;" />

            <button style="display: inline-block;padding:0px;" class="btn btn-sm btn-image send-message-open"
                type="submit" id="submit_message" data-id="{{ $issue->id }}" style="width:10%"><img
                    src="/images/filled-sent.png" /></button>
        </div>
        <div class="d-flex">
            <button type="button" class="btn btn-xs btn-image load-communication-modal" data-object='task'
                data-id="{{ $issue->id }}" style="mmargin-top: -0%;margin-left: -2%; width:10%"
                title="Load messages"><img src="/images/chat.png" alt=""></button>
            <span
                class="Website-task pr-0 {{ ($issue->message && $issue->message_status == 0) ||$issue->message_is_reminder == 1 ||($issue->sent_to_user_id == Auth::id() && $issue->message_status == 0)? '': '' }} justify-content-between expand-row-msg"
                style="word-break: break-all;margin-top:6px; width:100%; margin-right:-13px;"
                data-id="{{ $issue->id }}">
    ...
    <td class="communication-td devtask-com d-flex" style="border-bottom: none; display: block;">
        <!-- class="expand-row" -->
        <div class="d-flex">
            <select id="master_user_id" class="form-control change-task-status select2" data-id="{{ $issue->id }}"
            ...
            <button style="float:right;padding-right:0px;" type="button" class="btn btn-xs show-status-history"
                title="Show Status History" data-id="{{ $issue->id }}" data-type="task">
                <i class="fa fa-info-circle"></i>
            </button>
            @if ($issue->is_flagged == 1)
                <button type="button" class="btn pr-0 btn-image flag-task pd-5" data-type="task"
                    data-id="{{ $issue->id }}"><img src="{{ asset('images/flagged.png') }}" /
                        style="filter: grayscale(1);"></button>
            @else
                <button type="button" class="btn btn-image flag-task pd-5" data-type="task"
                    data-id="{{ $issue->id }}"><img src="{{ asset('images/unflagged.png') }}" /></button>
            @endif
        </div>
