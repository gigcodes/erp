@if (Auth::user()->isAdmin())
                ...
                <td>
                    ...
                    <a href="javascript:;" data-id="{{ $issue->id }}" class="upload-document-btn">
                        <img width="15px"
                            ...
                            <a href="javascript:;" data-id="{{ $issue->id }}" class="list-document-btn"><img
                                        width="15px" src="/images/archive.png" alt="" style="cursor: default;"><a>
                                    ...
                                    <a href="javascript:;" data-toggle="modal" data-target="#developmentReminderModal"
                                       ...
                                       data-reminder_last_reply="{{ $issue->reminder_last_reply }}">
                                        <i class="fa fa-bell @if(!empty($issue->reminder_message) && $issue->frequency > 0) {{ 'green-notification'  }} @else {{ 'red-notification' }} @endif"
                                           aria-hidden="true"></i>
                                    </a>

                                    <br>
                                    {{ \Carbon\Carbon::parse($issue->created_at)->format('H:i d-m') }}
                                    @if($issue->task_type_id == 1) Devtask @elseif($issue->task_type_id == 3)
                                        Issue @endif
                </td>
            @endif
            ...
            @if (!in_array('Milestone', $dynamicColumnsToShowDl))
                <td>
                    @if($issue->is_milestone)
                        ...
                        @else
                            <input type="number" name="milestone_completed" id="milestone_completed_{{$issue->id}}"
                                   placeholder="Completed..." class="form-control save-milestone"
                                   value="{{$issue->milestone_completed}}" data-id="{{$issue->id}}">
                        @endif
                    @else
                        No
                    @endif
        ...
        @if (!in_array('Actions', $dynamicColumnsToShowDl))
            <tr class="action-btn-tr-{{$issue->id}} d-none">
                ...
                <td colspan="15">
                    ...
                    <button class="btn btn-sm btn-image create-task-document" title="Create document"
                            data-id="{{$issue->id}}">
                        <i class="fa fa-file-text" aria-hidden="true"></i>
                    </button>
                    <button class="btn btn-sm btn-image show-created-task-document" title="Show created document"
                            data-id="{{$issue->id}}">
                        <i class="fa fa-list" aria-hidden="true"></i>
                    </button>
                    <button class="btn btn-sm btn-image add-document-permission" data-task_id="{{$issue->id}}"
    ...
    @else
        ...
        <tr class="action-btn-tr-{{$issue->id}} d-none">
            <td class="font-weight-bold">Action</td>
            <td colspan="15">
                <button class="btn btn-image set-remark" data-task_id="{{ $issue->id }}" data-task_type="Dev-task"><i
