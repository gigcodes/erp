...
@section('content')
    ...
    <div class="table-responsive">
        ...
        <tr>
            <th style="width: 8%"><a href="/order{{ isset($term) ? '?term='.$term.'&' : '?' }}sortby=id{{ ($orderby == 'asc') ? '&orderby=desc' : '' }}">ID</a></th>
            <th style="width: 8%"><a href="/order{{ isset($term) ? '?term='.$term.'&' : '?' }}sortby=date{{ ($orderby == 'asc') ? '&orderby=desc' : '' }}">Date</a></th>
            <th style="width: 12%"><a href="/order{{ isset($term) ? '?term='.$term.'&' : '?' }}sortby=order_handler{{ ($orderby == 'asc') ? '&orderby=desc' : '' }}">Order Handler</a></th>
            <th style="width: 12%"><a href="/order{{ isset($term) ? '?term='.$term.'&' : '?' }}sortby=client_name{{ ($orderby == 'asc') ? '&orderby=desc' : '' }}">Client Name</a></th>
            <th style="width: 8%"><a href="/order{{ isset($term) ? '?term='.$term.'&' : '?' }}sortby=status{{ ($orderby == 'asc') ? '&orderby=desc' : '' }}">Order Status</a></th>
