                ...
                <td  class="log-message-popup" data-log_message="{{ $contact->getLatestSocialContactThread->text }}">{{ substr($contact->getLatestSocialContactThread->text ,0,15) }}...</td>               <td class="message-input p-0 pt-2 pl-3">
                    <div class="cls_textarea_subbox">
                        <div class="btn-toolbar" role="toolbar">
                            <div class="w-75">
                                <textarea rows="1" class="form-control quick-message-field cls_quick_message addToAutoComplete" name="message" placeholder="Message" id="textareaBox_{{ $contact->id }}" data-customer-id="{{ $contact->id }}"></textarea>    
                            </div>
                            <div class="w-25 pl-2" role="group" aria-label="First group">
                                <button type="button" class="btn btn-sm m-0 p-0 mr-1 btn-image send-message1" data-id="textareaBox_{{ $contact->id }}">
                                    <img src="/images/filled-sent.png">
                                </button>
                                <button type="button" class="btn btn-sm m-0 p-0 mr-1 btn-image load-contact-communication-modal" data-object="social-contact" data-id="{{$contact->id}}" data-load-type="text" data-all="1" title="Load messages"><img src="{{asset('images/chat.png')}}" alt=""></button>
                            </div>
                        </div>
                    </div>
                </td>
...
<script>
    ...
    $(document).on('click', '.send-message1',function(){
        ...
        $.ajax({
            url: "{{ route('instagram.message.send') }}",
            method: 'POST',
            async: true,
            data: {
                _token: '{{ csrf_token() }}',
