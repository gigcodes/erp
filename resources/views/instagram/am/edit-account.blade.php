...
@section('content')
    <div class="row">
        <div class="col-md-12">
            <h2 class="page-heading"><a href="{{ action([\App\Http\Controllers\InstagramController::class, 'accounts']) }}">Back</a> | Edit Account: {{ $account->last_name }}</h2>
        </div>
    </div>
    <div class="row">
        <div class="p-5" style="background: #dddddd">
            <form action="{{ action([\App\Http\Controllers\InstagramController::class, 'update'], $account->id) }}" method="post">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-4">
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            <input {{ $account->broadcast ? 'checked' : '' }} type="checkbox" id="broadcast" name="broadcast">
                            <label for="broadcast">Direct Messaging</label>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <input {{ $account->manual_comment ? 'checked' : '' }} type="checkbox" id="manual_comments" name="manual_comments">
                            <label for="manual_comments">Manual Comments</label>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <input {{ $account->bulk_comment ? 'checked' : '' }} type="checkbox" id="bulk_comments" name="bulk_comments">
