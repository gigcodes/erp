@foreach ($websites as  $list)
    ...
    @if($stIndex==0)
    <tr class="expand-{{$list->id}} hidden">
         <td colspan="4" class="text-center"><h4>Stores</h4></td>
     </tr>
    @endif
    ...
    <td>
       ...
       <span class="btn" > <input type="checkbox" class="defaultInput" {{$store->is_default?'checked':''}} onclick="setStoreAsDefault(this)" data-website-id="{{$list->id}}" data-store-id="{{$store->id}}"/> Set as default</span>
