@foreach ($logScrappers as $logScrapper)
    <tr>
        <td style="width: 20% !important;">@if($logScrapper->brandLink($logScrapper->sku,$logScrapper->brand))<a href="{{ $logScrapper->brandLink($logScrapper->sku,$logScrapper->brand) }}" target="_blank"> @endif{{ $logScrapper->brand }}</a> <br> {{ $logScrapper->getSKUExample($logScrapper->brand) }}</td>
        <td style="width: 20% !important;">@if(isset($logScrapper->category)) {{ $logScrapper->dataUnserialize($logScrapper->category) }} @endif</td>
        <td style="width: 20% !important;">{{ $logScrapper->website }} <br> {{ $logScrapper->getSKUExampleFromLogScraper($logScrapper->website,$logScrapper->brand) }}</td>
