...
<tr>
    ...
    <td>
        ...
        @endif
        <br>
        <a title="{{$log->url}}" href="{{$log->url}}">{{ Str::limit($log->url, 30, '...')}}</a>
    </td>
    ...
    <td class="expand-row-msg" data-name="request_data" data-id="{{$log->id}}">
        <span class="show-short-request_data-{{$log->id}}">{{ Str::limit($log->request_data, 50, '...')}}</span>
    ...
    <td class="expand-row-msg" data-name="response_data" data-id="{{$log->id}}">
        <span class="show-short-response_data-{{$log->id}}">{{ Str::limit($log->response_data, 45, '...')}}</span>
