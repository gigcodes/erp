    ...
    @if ($images = $product->getMedia(config('constants.media_tags')))
      @foreach ($images as $image)
        <div class="col-md-3 col-xs-6 text-center mb-5">
          ...
          <a href="#" class="btn {{ in_array($image->getKey(), $selected_products) ? 'btn-success' : 'btn-secondary' }} attach-photo" data-image="{{ ($model_type == 'purchase-replace' || $model_type == 'broadcast-images') ? $product->id : $image->getKey() }}" data-attached="{{ in_array($image->getKey(), $selected_products) ? 1 : 0 }}">Attach</a>
        </div>
      @endforeach
    @else
      <div class="col-md-3 col-xs-6 text-center mb-5">
