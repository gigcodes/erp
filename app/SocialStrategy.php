...
class SocialStrategy extends Model
{
    ...
    {
        if ($needBroadcast) {
            return $this->hasMany(\App\ChatMessage::class, 'social_strategy_id')->where(function ($q) {
                $q->whereIn('status', ['7', '8', '9', '10'])->orWhere('group_id', '>', 0);
            })->latest();
        } else {
            return $this->hasMany(\App\ChatMessage::class, 'social_strategy_id')->whereNotIn('status', ['7', '8', '9', '10'])->latest();
        }
    }

    public function content()
    {
        return $this->hasOne(\App\User::class, 'id', 'content_id');
    }

    public function execution()
    {
        return $this->hasOne(\App\User::class, 'id', 'execution_id');
    }
}
