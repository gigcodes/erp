                ...
                foreach ($chat_messages as $chat_message) {
                    ...
                    if ($chat_message->hasMedia(config('constants.media_tags'))) {
                        ...
                        foreach ($chat_message->getMedia(config('constants.media_tags')) as $image) {
                            if ($error == 1) {
                                try {
                                    app(\App\Http\Controllers\WhatsAppController::class)->sendWithWhatsApp($chat_message->customer->phone, $chat_message->customer->whatsapp_number, str_replace(' ', '%20', getMediaUrl($image)), false, $new_message->id);
                                } catch (\Exception $e) {
                                }
                            }
                        }
