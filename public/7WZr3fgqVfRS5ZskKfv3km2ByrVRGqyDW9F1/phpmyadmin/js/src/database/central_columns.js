...
AJAX.registerOnload("database/central_columns.js", function () {
  $(
    "#tableslistcontainer input,#tableslistcontainer select,#tableslistcontainer .default_value,#tableslistcontainer .open_enum_editor"
  ).hide();
  ...
  $("#tableslistcontainer")
    .find('button[name="edit_central_columns"]')
    .on("click", function (event) {
      event.preventDefault();
      ...
      var editColumnData =
        ...
        "" +
        argsep +
        "edit_central_columns_page=true" +
        argsep +
        "ajax_request=true" +
        argsep +
        ...
        argsep +
        "db=" +
        encodeURIComponent(CommonParams.get("db")) +
        argsep +
        "server=" +
      ...
      AJAX.source = $(this);
      $.post(
        "index.php?route=/database/central-columns",
        editColumnData,
  ...
  $("#multi_edit_central_columns").on("submit", function (event) {
    ...
    var argsep = CommonParams.get("arg_separator");
    var multiColumnEditData =
      $("#multi_edit_central_columns").serialize() +
      argsep +
      "multi_edit_central_column_save=true" +
      argsep +
      "ajax_request=true" +
      argsep +
      "ajax_page_request=true" +
  ...
  $(".edit").on("click", function () {
    ...
    var attributeVal = $(
      "#f_" + rownum + " td[name=col_attribute] span"
    ).html();
    $(
      "#f_" +
        rownum +
  ...
  $(".del_row").on("click", function (event) {
    ...
    $td.confirm(question, null, function () {
      ...
      $("#del_col_name").val(
        "selected_fld%5B%5D=" + $("#checkbox_row_" + rownum).val()
      );
      $("#del_form").trigger("submit");
    });
  });
  ...
  $(".edit_save_form").on("click", function (event) {
    ...
    $.ajax({
      ...
      data:
        datastring + CommonParams.get("arg_separator") + "ajax_request=true",
      dataType: "json",
      success: function (data) {
        if (data.message !== "1") {
          ...
          $("#f_" + rownum + " td input[id=checkbox_row_" + rownum + "]")
            .val($("#f_" + rownum + " input[name=col_name]").val())
            .html();
          $("#f_" + rownum + " td[name=col_name] span")
            .text($("#f_" + rownum + " input[name=col_name]").val())
            .html();
          $("#f_" + rownum + " td[name=col_type] span")
            .text($("#f_" + rownum + " select[name=col_type]").val())
            .html();
          $("#f_" + rownum + " td[name=col_length] span")
            .text($("#f_" + rownum + " input[name=col_length]").val())
            .html();
          $("#f_" + rownum + " td[name=collation] span")
            .text($("#f_" + rownum + " select[name=collation]").val())
            .html();
          $("#f_" + rownum + " td[name=col_attribute] span")
            .text($("#f_" + rownum + " select[name=col_attribute]").val())
            .html();
          $("#f_" + rownum + " td[name=col_isNull] span")
            .text(
              $("#f_" + rownum + " input[name=col_isNull]").is(":checked")
                ? "Yes"
                : "No"
            )
            .html();
          $("#f_" + rownum + " td[name=col_extra] span")
            .text(
              $("#f_" + rownum + " input[name=col_extra]").is(":checked")
                ? "auto_increment"
                : ""
            )
            .html();
          $("#f_" + rownum + " td[name=col_default] span")
            .text($("#f_" + rownum + " :input[name=col_default]").val())
            .html();
        }
        $("#save_" + rownum).hide();
        $("#edit_" + rownum).show();
  ...
  $("#table-select").on("change", function () {
    ...
    $("#column-select").html(
      '<option value="">' + Messages.strLoading + "</option>"
    );
    if (selectValue !== "") {
