...
function changePluginOpts() {
  ...
  if (selectedPluginName === "csv") {
    importNotification.innerHTML =
      '<div class="alert alert-info mb-0 mt-3" role="alert">' +
      Messages.strImportCSV +
      "</div>";
  }
}
...
function matchFile(fname) {
  ...
  if (len !== 0) {
    ...
    if (
      $("select[name='format'] option").filterByValue(fnameArray[len - 1])
        .length === 1
    ) {
      $("select[name='format'] option")
        .filterByValue(fnameArray[len - 1])
        .prop("selected", true);
      changePluginOpts();
    }
  }
...
AJAX.registerOnload("import.js", function () {
  ...
  $(document).on("submit", "#import_file_form", function () {
    ...
    var fileMsg =
      '<div class="alert alert-danger" role="alert"><img src="themes/dot.gif" title="" alt="" class="icon ic_s_error"> ' +
