...
function prepareJSVersion() {
  ...
  $("#gis_editor")
    ...
    .before("<br><br>"); // Add close and cancel links
  ...
  ).insertAfter($("input[name='gis_data[save]']")); // Remove the unnecessary text
...
function insertDataAndClose() {
  ...
  $.post(
    "index.php?route=/gis-data-editor",
    $form.serialize() + argsep + "generate=true" + argsep + "ajax_request=true",
    function (data) {
      if (typeof data !== "undefined" && data.success === true) {
...
AJAX.registerOnload("gis_data_editor.js", function () {
  ...
  $(document).on("click", "#gis_editor a.addJs.addGeom", function () {
    ...
    var $geomType = $(
      "select[name='gis_data[" + (noOfGeoms - 1) + "][gis_type]']"
    ).clone();
    $geomType
