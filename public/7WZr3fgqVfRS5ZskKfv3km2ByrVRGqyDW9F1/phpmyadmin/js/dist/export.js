...
Export.createTemplate = function (name) {
  ...
  $.post(
    "index.php?route=/export/template/create",
    params,
    function (response) {
...
Export.updateTemplate = function (id) {
  ...
  Functions.ajaxShowMessage();
  $.post(
    "index.php?route=/export/template/update",
    params,
    function (response) {
      if (response.success === true) {
...
Export.deleteTemplate = function (id) {
  ...
  Functions.ajaxShowMessage();
  $.post(
    "index.php?route=/export/template/delete",
    params,
    function (response) {
      if (response.success === true) {
        $("#template")
...
AJAX.registerOnload("export.js", function () {
  ...
  $("#plugins").on("change", function () {
    $("#format_specific_opts")
      .find("div.format_specific_options")
      .addClass("d-none");
  ...
  $("input[type='radio'][name='sql_structure_or_data']").on(
    "change",
    function () {
      var commentsArePresent = $("#checkbox_sql_include_comments").prop(
        "checked"
      );
      var show = $(
        "input[type='radio'][name='sql_structure_or_data']:checked"
      ).val();
  ...
  $('input[type="checkbox"][name="as_separate_files"]').on(
    "change",
    function () {
      if ($(this).is(":checked")) {
        $("#compression").val("zip");
  ...
  $("#compression").on("change", function () {
    if ($("option:selected").val() !== "zip") {
      $('input[type="checkbox"][name="as_separate_files"]').prop(
        "checked",
        false
      );
    }
...
Export.setupTableStructureOrData = function () {
  ...
  var forceStructureOrData = !$("input[name='" + formElemName + "_default']")
    .length;
...
Export.toggleStructureDataOpts = function () {
  ...
  var show = $(
    "input[type='radio'][name='" + radioFormName + "']:checked"
  ).val(); // Show the #rows if 'show' is not structure
...
AJAX.registerOnload("export.js", function () {
  ...
  $("input[type='radio'][name='output_format']").on(
    "change",
    Export.toggleSaveToFile
  );
});
...
Export.handleAddProcCheckbox = function () {
  if (
    $("#table_structure_all").is(":checked") === true &&
...
AJAX.registerOnload("export.js", function () {
  ...
  $("input[type='radio'][name$='_structure_or_data']").on(
    "change",
    function () {
...
Export.toggleQuickOrCustom = function () {
  if (
    $("input[name='quick_or_custom']").length === 0 || // custom_no_form option
...
Export.checkTimeOut = function (timeLimit) {
  ...
  timeOut = setTimeout(function () {
    $.get(
      "index.php?route=/export/check-time-out",
      {
        ajax_request: true,
      },
      function (data) {
...
AJAX.registerOnload("export.js", function () {
  $("input[type='radio'][name='quick_or_custom']").on(
    "change",
