  ...
  $(document).on("click", "input:checkbox.checkall", function (e) {
    ...
    if (!e.shiftKey || lastClickedRow === -1) {
      ...
      $tr
        ...
        .find(":checkbox")
        .prop("checked", true)
        .trigger("change"); // remember the last shift clicked row
...
Functions.handleSimulateQueryButton = function () {
  var updateRegExp = new RegExp(
    "^\\s*UPDATE\\s+((`[^`]+`)|([A-Za-z0-9_$]+))\\s+SET\\s",
    "i"
  );
...
Functions.updateQueryParameters = function () {
  if ($("#parameterized").is(":checked")) {
    ...
    $.each(parameters, function (i, parameter) {
      ...
      if (!$param.length) {
        $param = $(
          '<span class="parameter" id="paramSpan_' + paramName + '"></span>'
        );
...
Functions.ajaxShowMessage = function (message, timeout, type) {
  ...
  var $retval = $(
    ...
    .html(msg)
    .show(); // If the notification is self-closing we should create a callback to remove it
          ...
          function (data) {
            if (typeof data !== "undefined" && data.success === true) {
              ...
              var tablesTable = $("#tablesForm")
                .find("tbody")
                .not("#tbl_summary_row"); // this is the first table created in this db
              ...
              if (tablesTable.length === 0) {
                ...
                data.newTableString = data.newTableString.replace(
                  /checkbox_tbl_/,
                  newLastRowId
                ); // append to table
              ...
              var params12 =
                "ajax_request=true" + argsep + "ajax_page_request=true";
              var tableStructureUrl =
                ...
                argsep +
                "goto=" +
                encodeURIComponent("index.php?route=/database/structure") +
                argsep +
                "table=" +
                data.params.table +
                "";
...
AJAX.registerOnload("functions.js", function () {
      ...
      .slider({
        ...
        slide: function (event, ui) {
          $(this)
            ...
            .val(Functions.sprintf(Messages.enum_addValue, ui.value));
        },
  ...
  $(document).on("click", "a.central_columns_dialog", function () {
    ...
    for (i = 0; i < min; i++) {
      ...
      fields +=
        Functions.escapeHtml(centralColumnList[db + "_" + table][i].col_extra) +
        "</span>" +
        "</div></td>";
...
Functions.indexDialogModal = function (
  ...
  $("#indexDialogModalGoButton").on("click", function () {
    ...
    $.post(
      $form.attr("action"),
      $form.serialize() + CommonParams.get("arg_separator") + "do_save_data=1",
      function (data) {
  ...
  if ($obj === undefined || !($obj instanceof jQuery) || $obj.length === 0) {
    ...
    if ($("#PMA_slidingMessage").length === 0) {
      $("#page_content").prepend(
        '<span id="PMA_slidingMessage" ' + 'class="d-inline-block"></span>'
      );
    }
...
$(function () {
  if (
    ...
    $("#floating_menubar")
      ...
      .append($("#topmenucontainer")); // Allow the DOM to render, then adjust the padding on the body
