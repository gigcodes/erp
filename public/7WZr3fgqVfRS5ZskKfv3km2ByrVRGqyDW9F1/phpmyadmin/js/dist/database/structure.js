...
DatabaseStructure.adjustTotals = function () {
  ...
  $summary
    .find(".tbl_num")
    .text(Functions.sprintf(Messages.strNTables, tableSum));
  ...
  $summary
    .find(".tbl_overhead")
    .text(overheadSum + " " + byteUnits[overheadMagnitude]);
};
/**
    ...
    success: function (response) {
      if (response.success) {
        ...
        if (response.real_row_count_all) {
          $.each(
            JSON.parse(response.real_row_count_all),
            function (index, table) {
              // Update each table row count.
              $('table.data td[data-table*="' + table.table + '"]').text(
                table.row_count
              );
            }
          );
...
AJAX.registerOnload("database/structure.js", function () {
  ...
  $("select[name=submit_mult]").on("change", function (event) {
    ...
    if (action === "make_consistent_with_central_list") {
      ...
      $("#makeConsistentWithCentralListModal")
        .modal("show")
  ...
  $(document).on("click", "a.truncate_table_anchor.ajax", function (event) {
    ...
    var question =
      Messages.strTruncateTableStrongWarning +
      " " +
      Functions.sprintf(
        Messages.strDoYouReally,
        "TRUNCATE `" + Functions.escapeHtml(currTableName) + "`"
      ) +
  ...
  $(document).on("click", "a.drop_table_anchor.ajax", function (event) {
    ...
    if (!isView) {
      question =
        Messages.strDropTableStrongWarning +
        " " +
        Functions.sprintf(
          Messages.strDoYouReally,
          "DROP TABLE `" + Functions.escapeHtml(currTableName) + "`"
        );
    } else {
      question = Functions.sprintf(
        Messages.strDoYouReally,
        "DROP VIEW `" + Functions.escapeHtml(currTableName) + "`"
      );
    }
