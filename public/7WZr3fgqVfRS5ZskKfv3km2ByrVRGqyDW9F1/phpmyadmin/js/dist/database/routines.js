...
const DatabaseRoutines = {
  ...
  exportDialog: function ($this) {
    ...
    function showExport(data) {
      if (data.success === true) {
        ...
        data.message =
          '<textarea cols="40" rows="15" class="w-100">' +
          data.message +
          "</textarea>";
    ...
    $.get(
      ...
      function (data) {
        if (data.success === true) {
          ...
          that.$ajaxDialog = $(
            '<div id="rteDialog">' + data.message + "</div>"
          ).dialog({
            width: 700,
            minWidth: 500,
            ...
            open: function () {
              ...
              if (
                $("#rteDialog").parents(".ui-dialog").height() >
                $(window).height()
              ) {
          ...
          that.syntaxHiglighter = Functions.getSqlEditor(
            $elm,
            {},
            "both",
            linterOptions
  ...
  reindexParameters: function () {
     ...
     */
    var index = 0;
    $("table.routine_params_table tbody")
      .find("tr")
      .each(function () {
        $(this)
          .find(":input")
  ...
  validateCustom: function () {
    ...
    var inputname = "";
    this.$ajaxDialog
      .find("table.routine_params_table")
      .last()
      .find("tr")
      .each(function () {
        ...
        if (isSuccess) {
          $(this)
            ...
            .each(function () {
              ...
              if (
                inputname.substr(0, 14) === "item_param_dir" ||
    ...
    if ($("select[name=item_type]").find(":selected").val() === "FUNCTION") {
      ...
      if (
        this.$ajaxDialog
          .find("table.rte_table")
          .find("textarea[name=item_definition]")
          .val()
          .toUpperCase()
          .indexOf("RETURN") < 0
      ) {
    ...
    $.post($this.attr("href"), params, function (data) {
      if (data.success === true) {
        ...
        if (data.dialog) {
          ...
          $ajaxDialog
            .find("input.datefield, input.datetimefield")
            .each(function () {
...
AJAX.registerOnload("database/routines.js", function () {
  ...
  $(document).on("change", "select[name=item_type]", function () {
    $(this)
      .closest("table")
      .find("tr.routine_return_row, .routine_direction_cell")
      .toggle();
  });
  ...
  $(document).on("click", "input[name=routine_addparameter]", function (event) {
    ...
    const $routineParamsTable = $(this)
      .closest("div.ui-dialog")
      .find(".routine_params_table");
    /**
     * @var new_param_row A string containing the HTML code for the
     *                    new row for the routine parameters table
     */

    const newParamRow = DatabaseRoutines.paramTemplate.replace(
      /%s/g,
      $routineParamsTable.find("tr").length - 1
    ...
    if (
      $(this)
        .closest("div.ui-dialog")
        .find("table.rte_table select[name=item_type]")
        .val() === "FUNCTION"
    ) {
    ...
    const $newrow = $(this)
      ...
      .find("tr")
      .has("td")
      .last(); // Enable/disable the 'options' dropdowns for parameters as necessary
