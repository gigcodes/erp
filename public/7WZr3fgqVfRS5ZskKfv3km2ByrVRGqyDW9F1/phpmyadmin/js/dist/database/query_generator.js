...
function generateCondition(criteriaDiv, table) {
  ...
  query +=
    ...
    Functions.escapeBacktick(
      table.siblings(".columnNameSelect").first().val()
    ) +
    "`";
  ...
  if (criteriaDiv.find(".criteria_rhs").first().val() === "text") {
    ...
    query += sprintf(
      formatsText[criteriaDiv.find(".criteria_op").first().val()],
      Functions.escapeSingleQuote(
        criteriaDiv.find(".rhs_text_val").first().val()
      )
    );
  } else {
    ...
    query +=
      "`" +
      Functions.escapeBacktick(
        criteriaDiv.find(".columnNameSelect").first().val()
      ) +
      "`";
  }
...
function generateJoin(newTable, tableAliases, fk) {
  ...
  if (tableAliases[fk.TABLE_NAME][0] !== "") {
    ...
    query +=
      " ON `" + Functions.escapeBacktick(tableAliases[fk.TABLE_NAME][0]) + "`";
  } else {
  ...
  if (tableAliases[fk.REFERENCED_TABLE_NAME][0] !== "") {
    query +=
      " = `" +
      Functions.escapeBacktick(tableAliases[fk.REFERENCED_TABLE_NAME][0]) +
      "`";
  } else {
...
function existReference(table, fk, usedTables) {
  var isReferredBy =
    fk.TABLE_NAME === table && usedTables.includes(fk.REFERENCED_TABLE_NAME);
  var isReferencedBy =
    fk.REFERENCED_TABLE_NAME === table && usedTables.includes(fk.TABLE_NAME);
