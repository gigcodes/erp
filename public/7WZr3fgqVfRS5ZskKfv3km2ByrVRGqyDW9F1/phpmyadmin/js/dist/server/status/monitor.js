...
function serverResponseError() {
  ...
  $("#emptyDialog").html(
    Functions.getImage("s_attention") + Messages.strInvalidResponseExplanation
  );
  $("#emptyDialog").dialog({
    buttons: btns,
  });
...
AJAX.registerOnload("server/status/monitor.js", function () {
  ...
  serverTimeDiff =
    new Date().getTime() - $jsDataForm.find("input[name=server_time]").val();
...
AJAX.registerOnload("server/status/monitor.js", function () {
  ...
  $('div.popupContent select[name="gridChartRefresh"]').on(
    ...
    function () {
      ...
      runtime.xmin =
        new Date().getTime() -
        serverTimeDiff -
        runtime.gridMaxPoints * monitorSettings.gridRefresh; // fixing chart shift towards left on refresh rate change
  ...
  $('input[name="chartType"]').on("change", function () {
    ...
    if (
      ...
      title ===
        $(
          'label[for="' + $('input[name="chartTitle"]').data("lastRadio") + '"]'
        ).text()
    ) {
  ...
  $('a[href="#submitAddSeries"]').on("click", function (event) {
    ...
    var str =
      serie.display === "differential" ? ", " + Messages.strDifferential : "";
  ...
  function calculateChartSize() {
    ...
    if (windowWidth > 768) {
      wdt =
        (panelWidth - monitorSettings.columns * chartSpacing.width) /
        monitorSettings.columns;
    }
          ...
          for (var j = 0; j < elem.nodes.length; j++) {
            ...
            if (elem.nodes[j].transformFn) {
              value = chartValueTransform(
                ...
                chartData[key][j], // Check if first iteration (oldChartData==null), or if newly added chart oldChartData[key]==null
                oldChartData === null ||
            ...
            if (value !== undefined) {
              ...
              if (
                elem.chart.series[j].data.length > runtime.gridMaxPoints &&
  ...
  function loadLogStatistics(opts) {
    ...
    function filterQueries(varFilterChange) {
      ...
      $("#logTable")
        .find("table tbody tr")
        .children("td")
        .eq(runtime.logDataCols.length - 2)
        .each(function () {
      ...
      if (varFilterChange) {
        ...
        setTimeout(function () {
          $("#logTable")
            .find("table")
            .trigger("sorton", [[[runtime.logDataCols.length - 1, 1]]]);
        }, 0);
  ...
  function buildLogTable(data, groupInserts) {
    ...
    var cols = [];
    var $table = $(
      '<table class="table table-light table-striped table-hover align-middle sortable"></table>'
    );
    var $tBody;
    var $tRow;
    var $tCell;
    ...
    for (var i = 0, l = rows.length; i < l; i++) {
      ...
      for (var j = 0, ll = cols.length; j < ll; j++) {
        ...
        if (j === cols.length - 2 && rows[i][cols[j]].match(/^SELECT/i)) {
          ...
          $tRow.append(
            "<td>" + formatValue(cols[j], rows[i][cols[j]]) + "</td>"
          );
        }
    ...
    if (
      $("#logTable").find("tr").first().find("th").last().text().indexOf("#") >
      -1
    ) {
      $("#logTable")
        .find("tr")
      ...
      if (groupInserts) {
        tooltipContent +=
          "<p>" + Messages.strMoreCountColumnExplanation + "</p>";
      }
  ...
  function loadQueryAnalysis(rowData) {
    ...
    $.post(
      ...
      function (responseData) {
        ...
        explain +=
          "<p><b>" + Messages.strAffectedRows + "</b> " + data.affectedRows;
        $("#queryAnalyzerDialog")
        ...
        if (data.profiling) {
          var chartData = [];
          var numberTable =
            '<table class="table table-sm table-light table-striped table-hover w-auto queryNums"><thead><tr><th>' +
            Messages.strStatus +
            "</th><th>" +
            Messages.strTime +
            "</th></tr></thead><tbody>";
