...
var DragDropImport = {
  ...
  getExtension: function (file) {
    ...
    if (
      jQuery.inArray(ext.toLowerCase(), DragDropImport.allowedExtensions) !== -1
    ) {
      return ext;
    }
  ...
  sendFileToServer: function (formData, hash) {
    ...
    $(
      '.pma_sql_import_status div li[data-hash="' + hash + '"] span.filesize'
    ).html(
      '<span hash="' +
        hash +
    ...
    $(
      ...
      if ($(this).attr("task") === "cancel") {
        ...
        $(this).children("span").html() === Messages.dropImportMessageFailed
      ) {
        // -- view information
        var $this = $(this);
  ...
  importFinished: function (hash, aborted, status) {
    ...
    if (!aborted) {
      if (status) {
        ...
        $(
          ...
          '<span class="underline">' + Messages.dropImportMessageFailed + "</a>"
        );
    ...
    $(
      '.pma_sql_import_status div li[data-hash="' +
        hash +
        '"] span.filesize span.pma_drop_file_status'
    ).attr("task", "info"); // Set icon

    $('.pma_sql_import_status div li[data-hash="' + hash + '"]').prepend(
      '<img src="./themes/dot.gif" title="finished" class="' + icon + '"> '
    ...
    if (dbname !== "") {
      ...
      for (var i = 0; i < files.length; i++) {
        ...
        if (ext !== "") {
          ...
          $('.pma_sql_import_status div li[data-hash="' + hash + '"]').append(
            '<br><progress max="100" value="2"></progress>'
          ); // uploading
