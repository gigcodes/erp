...
DesignerMove.mouseMove = function (e) {
  ...
  if (curClick !== null) {
    ...
    var curX = parseFloat(
      $curClick.attr("data-" + mainDirection) || $curClick.css(mainDirection)
    );
    ...
    for (key in contr[K]) {
      ...
      for (key2 in contr[K][key]) {
        ...
        for (key3 in contr[K][key][key2]) {
          ...
          var x2Right =
            x2Left +
            document.getElementById(contr[K][key][key2][key3][0]).offsetWidth;
...
DesignerMove.addOtherDbTables = function () {
  ...
  $("#designerModalGoButton").on("click", function () {
    ...
    var $table = $(
      '[id="' + encodeURIComponent(db) + "." + encodeURIComponent(table) + '"]'
    );
    ...
    if ($(this).val()) {
      ...
      $.post(
        ...
        function (data) {
          ...
          if (rows.length === 0) {
            $selectTable.append(
              '<option value="">' + Messages.strNone + "</option>"
            );
          }
  ...
  for (var key in jTabs) {
    ...
    document.getElementById("t_v_" + key + "_").value =
      document.getElementById("id_tbody_" + key).style.display === "none"
        ? 0
        : 1;
    document.getElementById("t_h_" + key + "_").value = document.getElementById(
      "check_vis_" + key
    ).checked
      ? 1
      : 0;
...
DesignerMove.clickField = function (db, T, f, pk) {
  ...
  if (onRelation) {
    if (!clickField) {
      ...
      var left =
        globX -
        (document.getElementById("layer_new_relation").offsetWidth >> 1);
      document.getElementById("layer_new_relation").style.left = left + "px";
      var top =
        globY - document.getElementById("layer_new_relation").offsetHeight;
...
DesignerMove.canvasClick = function (id, event) {
  ...
  if (selected) {
    // select relations
    var left =
      globX - (document.getElementById("layer_upd_relation").offsetWidth >> 1);
    document.getElementById("layer_upd_relation").style.left = left + "px";
    var top =
      globY - document.getElementById("layer_upd_relation").offsetHeight - 10;
...
DesignerMove.generalScroll = function () {
  ...
  timeoutId = setTimeout(function () {
    ...
    document.getElementById("top_menu").style.left =
      document.body.scrollLeft + "px"; // eslint-disable-next-line compat/compat
