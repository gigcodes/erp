...
AJAX.registerOnload("table/structure.js", function () {
  ...
  $(document).on("click", "a.drop_column_anchor.ajax", function (event) {
    ...
    var question = Functions.sprintf(
      ...
      "ALTER TABLE `" + currTableName + "` DROP `" + currColumnName + "`;"
    );
    var $thisAnchor = $(this);
      ...
      $.post(url, params, function (data) {
        if (typeof data !== "undefined" && data.success === true) {
          ...
          $("select[name=after_field]")
            ...
            .remove(); // by default select the (new) last option to add new column
          // (in case last column is dropped)

          $("select[name=after_field] option")
            .last()
            .attr("selected", "selected"); // refresh table stats
  ...
  $(document).on("click", "#move_columns_anchor", function (e) {
    ...
    $("#designerModalGoButton").on("click", function () {
      ...
      $.post(
        $form.prop("action"),
        serialized + CommonParams.get("arg_separator") + "ajax_request=true",
        function (data) {
          if (data.success === false) {
            ...
            for (var i in data.columns) {
              ...
              var $theRow = $rows
                ...
                .closest("tr"); // append the row for this column to the table
  ...
  $(document).on("click", "a[id^=partition_action].ajax", function (e) {
    ...
    function submitPartitionAction(url) {
      var params =
        "ajax_request=true&ajax_page_request=true&" + $link.getPostData();
      Functions.ajaxShowMessage();
      AJAX.source = $link;
      $.post(url, params, AJAX.responseHandler);
    }
