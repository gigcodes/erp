...
function nullify(theType, urlField, md5Field, multiEdit) {
  ...
  if (
    ...
    rowForm.elements["funcs" + multiEdit + "[" + md5Field + "]"].selectedIndex =
      -1;
  ...
  if (Number(theType) === 1) {
    ...
    ][1].selectedIndex = -1; // Other "ENUM" field
    ...
    rowForm.elements[
      "fields" + multiEdit + "[" + md5Field + "][]"
    ].selectedIndex = -1; // Foreign key field (drop-down)
  } else if (Number(theType) === 4) {
    rowForm.elements[
      "fields" + multiEdit + "[" + md5Field + "]"
    ].selectedIndex = -1; // foreign key field (with browsing icon for foreign values)
...
function verificationsAfterFieldChange(urlField, multiEdit, theType) {
  ...
  var $thisInput = $(
    ":input[name^='fields[multi_edit][" + multiEdit + "][" + urlField + "]']"
  ...
  $(
    "input[name='fields_null[multi_edit][" + multiEdit + "][" + urlField + "]']"
  ).prop("checked", false); // Unchecks the Ignore checkbox for the current row
  ...
  $('select[name="submit_type"]').on("change", function () {
    ...
    autoIncrementColumn.each(function () {
      ...
      var valueField = $table.find(
        'input[name="' + thisElemName.replace("auto_increment", "fields") + '"]'
      );
  ...
  if (currRows < targetRows) {
    var tempIncrementIndex = function () {
      ...
      if (
        ...
        if (
          $thisElement.closest("tr").find("span.column_type").html() !== "enum"
        ) {
          $thisElement.val(
            $thisElement.closest("tr").find("span.default_value").html()
          );
        }

        $thisElement
          .off("change") // Remove onchange attribute that was placed
    ...
    while (currRows < targetRows) {
      ...
      if (currRows === 1) {
        $(
          ...
          .after(
            '<label for="insert_ignore_1">' + Messages.strIgnore + "</label>"
          );
      } else {
        /**
