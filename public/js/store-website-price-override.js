var page = {
  ...
  calculatePriceDuty: function (ele) {
    ...
    var _z = {
      url:
        typeof href != "undefined" ? href : this.config.mainUrl + "/calculate",
      method: "get",
      data: form.serialize(),
      beforeSend: function () {
        $("#loading-image").show();
        $(".calculated-result-display")
          .removeClass("btn-secondary btn")
          .html("");
      },
    };
  ...
  displayCalculation: function (response) {
    ...
    if (response.code == 200 && response.data.total > 0) {
      var duty = parseInt(
        (parseInt(response.data.total) * parseInt(response.data.duty)) / 100
      );
      var total = parseInt(parseInt(response.data.total) + parseInt(duty));
      $(".calculated-result-display")
        .addClass("btn-secondary btn")
        .html(
          "Original Price :" +
