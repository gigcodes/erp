  ...
  init: function (settings) {
    ...
    $(document).on("click", ".imgAdd", function (e) {
      $(this)
        ...
        .find(".imgAdd")
        .before(
          '<div class="col-sm-3 imgUp"><div class="imagePreview"></div><label class="btn btn-primary">Upload<input type="file" name="files[]" class="uploadFile img" value="Upload Photo" style="width:0px;height:0px;overflow:hidden;"></label><i class="fa fa-times del"></i></div>'
        );
    });
    ...
    $(document).on("click", ".create-product-template", function (e) {
      if ($("#product-template-from").valid()) {
        var noOfImages = $("#product-template-from")
          .find(".template_no option:selected")
          .data("no-of-images");
        var uploadImage = 0;

        $("#product-template-from")
          .find(".product_media_list")
          .each(function () {
    ...
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.baseUrl +
            "/product-templates/response?keyword=" +
            search,
      method: "get",
    };
  ...
  submitForm: function (ele) {
    ...
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.baseUrl + "/product-templates/create",
      method: "post",
      data: formData,
    };
