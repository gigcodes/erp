...
function updateUserFlagForTaskPlan(ele) {
    ...
    if (confirm(btn.data('is_task_planned') == 1 ? 'Are you sure? Do you want unflagged this user for task plan?' : 'Are you sure? Do you want flagged this user for task plan?')) {
        jQuery.ajax({
            headers: {
                'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')
            },
