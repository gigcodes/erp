var page = {
  init: function (settings) {
    ...
    page.config.mainUrl = page.config.baseUrl + "/manage-task-category";
  ...
  deleteResults: function (response) {
    if (response.code == 200) {
      this.getResults();
      toastr["success"]("Task Category deleted successfully", "success");
    } else {
      $("#loading-image").hide();
  ...
  submitForMerge: function (ele) {
    var selectedIds = [];
    $(".manage-category-ckbx").each(function (k, v) {
      if ($(v).is(":checked")) {
