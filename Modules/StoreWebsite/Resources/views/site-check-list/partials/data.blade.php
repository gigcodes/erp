         ...
         @foreach($store_websites as $key2=>$sw) 
            ...
            <td>@if($check)
				...
				<div class="col-md-12 mb-1 p-0 d-flex  pt-2 mt-1">
					 <input style="margin-top: 0px;width:auto !important;" type="text" class="form-control quick-message-field" name="message" placeholder="Message" value="" id="remark_{{$sdc->id.$sw->id}}" data-catId="{{ $sdc->id }}" data-siteId="@if($site){{ $site->id }}@endif" data-websiteId="@if($website) {{ $website->id }} @endif" >
					 <div style="margin-top: 0px;" class="d-flex p-0">
						<button class="btn pr-0 btn-xs btn-image " onclick="saveRemarks({{$sdc->id.$sw->id}})"><img src="/images/filled-sent.png" /></button>
					 </div>
                </div>
