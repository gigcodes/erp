    ...
    ?>
        ...
        <tr>
            ...
            <td class="expand-row-msg" data-name="cat_title" data-id="{{$pagrank}}">
                ...
                @include(
                    'storewebsite::site-development.partials.site-asset-modal'
                )
                @include(
            ...
            <td class="pt-0 pr-2">
                <div class="col-md-12 mb-1 p-0 d-flex pt-2 mt-1">
                    ...
                    <div style="margin-top: 0px;" class="d-flex p-0">
                        <button class="btn pr-0 btn-xs btn-image " onclick="saveRemarks({{ $key }})"><img
            ...
            <td style="display:none;">
                <input type="hidden" id="website_id"
                    value="{{ isset($website) ? $website->id : $category->website_id }}">
                <input style="margin-top: 0;" type="text" class="form-control save-item"
                    data-category="{{ $category->id }}" data-type="title"
                    value="@if ($site) {{ $site->title }} @endif"
                    data-site="@if ($site) {{ $site->id }} @endif">
                <form>
                    <label class="radio-inline">
                        <input class="save-artwork-status" type="radio" name="artwork_status"
            ...
            <td style="display:none;">
                <div class="row m-0">
                    <div class="col-md-6 mb-1 pr-0 pl-0">
                        <select style="margin-top: 5px;"
                            class="form-control save-item-select developer assign-to select2"
                            data-category="{{ $category->id }}" data-type="developer"
                            data-site="@if ($site) {{ $site->id }} @endif"
                            name="developer_id"
                            id="user-@if ($site) {{ $site->id }} @endif">
                            <option value="">Select Developer</option>
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}"
                                    @if ($site && $site->developer_id == $user->id) selected @endif>{{ $user->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6 mb-1 pl-2 pr-2">
                        <select style="margin-top: 5px;" name="designer_id"
                            class="form-control save-item-select designer assign-to select2"
                            data-category="{{ $category->id }}" data-type="designer_id"
                            data-site="@if ($site) {{ $site->id }} @endif"
                            id="user-@if ($site) {{ $site->id }} @endif">
                            <option value="">Select Designer</option>
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}"
                                    @if ($site && $site->designer_id == $user->id) selected @endif>{{ $user->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="row m-0">
                    <div class="col-md-6 mb-1 pr-0 pl-0">
                        <select style="margin-top: 5px;" name="html_designer"
                            class="form-control save-item-select html assign-to select2"
                            data-category="{{ $category->id }}" data-type="html_designer"
                            data-site="@if ($site) {{ $site->id }} @endif"
                            id="user-@if ($site) {{ $site->id }} @endif">
                            <option value="">Select Html</option>
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}"
                                    @if ($site && $site->html_designer == $user->id) selected @endif>{{ $user->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6 mb-1 pl-2 pr-2">
                        <select style="margin-top: 5px;" name="tester_id"
                            class="form-control save-item-select html assign-to select2"
                            data-category="{{ $category->id }}" data-type="tester_id"
                            data-site="@if ($site) {{ $site->id }} @endif"
                            id="user-@if ($site) {{ $site->id }} @endif">
                            <option value="">Select Tester</option>
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}"
                                    @if ($site && $site->tester_id == $user->id) selected @endif>{{ $user->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <td class="pt-1">
                <button type="button" data-site-id="@if ($site) {{ $site->id }} @endif"
                    ...
                    data-store-website-id="{{ isset($website) ? $website->id : $category->website_id }} "
                ...
                @if ($site)
                    ...
                    <button type="button" title="Artwork status history" class="btn artwork-history-btn pd-5"
