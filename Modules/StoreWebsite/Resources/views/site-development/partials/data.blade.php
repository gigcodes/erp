	...
	<tr>
		...
		<td>{{ $category->title }} <br><button onclick="editCategory({{$category->id}})" style="background-color: transparent;border: 0;"><i class="fa fa-edit"></i></button></td>
			<td>@if($website) {{ $website->website }} @endif</td>
			<input type="hidden" id="website_id" value="@if($website) {{ $website->id }} @endif">
			<td><input type="text" class="form-control save-item" data-category="{{ $category->id }}" data-type="title" value="@if($site) {{ $site->title }}@endif" data-site="@if($site) {{ $site->id }}@endif"></td>
			...
			<td><select class="form-control save-item-select" data-category="{{ $category->id }}" data-type="status" data-site="@if($site) {{ $site->id }}@endif">
  				<option value="1" @if($site && $site->status == 1) selected @endif >Active</option>
  				<option value="2" @if($site && $site->status == 2) selected @endif >InActive</option>
  			</select>
			</td>
			<td>
				...
				@if($site)
					...
					<div class="chat_messages expand-row table-hover-cell">
					    <span class="chat-mini-container"> 
					      @if($site->lastChat) {{ $site->lastChat->message }} @endif
					    </span>
