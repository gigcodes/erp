    ...
    <div overlay v-cloak @click="hide">
        <div class="popup-body" tabindex="-1" @click.stop>
            ...
            <div class="popup-header primary-background">
                <div class="popup-title">{{ trans('bookstack::components.image_select') }}</div>
                ...
                <div class="image-manager-content">
                    ...
                    <div>
                        <form @submit.prevent="searchImages" class="contained-search-box">
                            <input placeholder="{{ trans('bookstack::components.image_search_hint') }}" v-model="searchTerm">
                            <button :class="{active: searching}" title="{{ trans('bookstack::common.search_clear') }}" type="button" @click="cancelSearch()" class="text-button cancel">@icon('close')</button>
                            <button title="{{ trans('bookstack::common.search') }}" class="text-button">@icon('search')</button>
                        </form>
                    </div>
                        ...
                        <div v-if="images.length > 0" v-for="(image, idx) in images">
                            <div class="image anim fadeIn" :style="{animationDelay: (idx > 26) ? '160ms' : ((idx * 25) + 'ms')}"
                                ...
                                <div class="image-meta">
                                    ...
                                    <span class="date">{{ trans('bookstack::components.image_uploaded', ['uploadedDate' => "{{ getDate(image.created_at) }" . "}"]) }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="load-more" v-show="hasMore" @click="fetchData">{{ trans('bookstack::components.image_load_more') }}</div>
                    </div>
                </div>
                        ...
                        <div class="image-manager-details anim fadeIn" v-if="selectedImage">
                            ...
                            <form @submit.prevent="saveImageDetails">
                                ...
                                <div class="form-group">
                                    <label for="name">{{ trans('bookstack::components.image_image_name') }}</label>
                            ...
                            <div class="clearfix">
                                ...
                                <button class="button anim fadeIn float right" v-show="selectedImage" @click="callbackAndHide(selectedImage)">
                                    {{ trans('bookstack::components.image_select_image') }}
                                </button>
                                <div class="clearfix"></div>
                                <div v-show="dependantPages">
                                    <p class="text-neg text-small">
                                        {{ trans('bookstack::components.image_delete_used') }}
                                    </p>
                                    <ul class="text-neg">
                                ...
                                <div v-show="deleteConfirm" class="text-neg text-small">
                                    {{ trans('bookstack::components.image_delete_confirm') }}
                                </div>
                            </div>
