...
jobs=$(curl -sSL --user "$USERNAME:$PASSWORD" "$JENKINS_URL/api/json" | jq -r '.jobs[].name')
echo $jobs
#for job in $jobs; do
curl -sSL --user "$USERNAME:$PASSWORD" "$JENKINS_URL/api/json" | jq -r '.jobs[].name' | while read job;
do
	echo "$job"
	last_build=$(curl -sSL --user "$USERNAME:$PASSWORD" "$JENKINS_URL/job/$job/api/json" | jq -r '.lastBuild.number')
	log_text=$(curl -sSL --user "$USERNAME:$PASSWORD" "$JENKINS_URL/job/$job/$last_build/consoleText")
	escaped_log_text=$(echo "$log_text" | sed "s/'//g")
	escaped_log_text=$(echo "$escaped_log_text" | sed "s/\"//g")
	escaped_log_text=$(echo "$escaped_log_text" | sed "s/\n/ /g")
	WARNING=`curl -sSL --user "$USERNAME:$PASSWORD" "$JENKINS_URL/job/$job/$last_build/consoleText" | grep -i "undefined\|error" | grep -v installing | grep -v error_email | grep -v error-handler `
	ERROR=`curl -sSL --user "$USERNAME:$PASSWORD" "$JENKINS_URL/job/$job/$last_build/consoleText" | grep "ERROR: Task"`
...
DUPLICATE=`mysql -h $DB_HOST -u $DB_USER -p$DB_PASSWORD -D $DB_NAME -N -se "select id from monitor_jenkins_builds where build_number='$last_build'"`
if [ -z "$DUPLICATE" ]
then
