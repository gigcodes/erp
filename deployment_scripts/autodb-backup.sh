...
magento_servers='BRANDS AVOIRCHIC OLABESL SOLOLUXURY SUVANDNAT THEFITEDIT THESHADESSHOP UPEAU VERALUSSO'
mageroot=/mnt/volume_blr1_02/magento_backup
magentobkp=$mageroot/$date
...
do
	...
	mysqldump -h $host -u $user -p"$pass" $db |gzip -v > $magentobkp/$server/$db.gz
done
